...
void gcmalloc::check_chunk(mArenaPtr ap, mChunkPtr cp) {
    unsigned long sz = cp->get_size();
    /* min and max possible addresses assuming contiguous allocation */
    char *max_address = (char *) (ap->topChunk) + ap->topChunk->get_size();
    ...
    if (!cp->is_mmaped()) {
        ...
        if (cp != ap->topChunk) {
            if (ap->contiguous()) {
                ...
                assert(((char *) cp + sz) <= ((char *) (ap->topChunk)));
            }
        } else {
        ...
        if (ap->contiguous() && ap->topChunk != ap->initial_top()) {
        ...
        /* mem is aligned */
        assert((cp->is_aligned())); // since this method always return true, this may have error
    }
}

void gcmalloc::check_free_chunk(mArenaPtr ap, mChunkPtr cp) {
    size_t sz = cp->size & ~(PREV_INUSE | NON_MAIN_ARENA);
    ...
    if ((unsigned long) (sz) >= MINSIZE) {
        ...
        assert(cp->prev_inuse());
        assert (next == ap->topChunk || next->inuse());
...
void gcmalloc::check_malloc_state(mArenaPtr ap) {
    ...
    if (ap->topChunk == nullptr || ap->topChunk == ap->initial_top())
        return;
    ...
    if (ap == &main_arena && ap->contiguous())
        assert((char *) gcmp.sbrk_base + ap->system_mem ==
               (char *) ap->topChunk + ap->topChunk->get_size());

    /* properties of fastbins */

    /* max_fast is in allowed range */
    /* We hard code it, so no need to check */
//    assert((DEFAULT_MXFAST & ~1) <= req2size(MAX_FAST_SIZE));

    max_fast_bin = (ap->fast_bins).index(DEFAULT_MXFAST);
...
void *gcmalloc::malloc_from_sys(size_t nb, mArenaPtr ap) {
    mChunkPtr old_top;      /* incoming value of ap->topChunk */
    ...
    */
    ...
    if (ap != &main_arena) {
        ...
        if ((long) (MINSIZE + nb - old_size) > 0
            ...
            /* Set up the new top.  */
            ap->topChunk = offset2Chunk(heap, sizeof(*heap));
            ap->topChunk->set_head((heap->size - sizeof(*heap)) | PREV_INUSE);
                ...
                if (ap->contiguous()) {
                    ...
                    if (front_misalign > 0) {
                        /*
                            ...
                            prev_inuse of ap->topChunk (and any chunk created from its start)
                    ...
                    */
                    ...
                    if (snd_brk != (char *) (-1)) {
                        ap->topChunk = (mChunkPtr) aligned_brk;
                        ap->topChunk->set_head((snd_brk - aligned_brk + correction) | PREV_INUSE);
...
int gcmalloc::trim_to_sys(size_t pad, mArenaPtr ap) {
    ...
    if (extra > 0) {
        ...
        */
        ...
        if (current_brk == (char *) (ap->topChunk) + top_size) {
            ...
            */
            ...
            if (new_brk != (char *) -1) {
                ...
                if (released != 0) {
                    ...
                    ap->topChunk->set_head((top_size - released) | PREV_INUSE);
                    check_malloc_state(ap);
                    return 1;
                }
            }
...
void *gcmalloc::malloc(size_t bytes) {
    ...
    if (!victim) {
        ...
        ar_ptr->mutex.unlock();
    assert(!victim || (mem2chunk(victim)->is_mmaped()) ||
...
void gcmalloc::free(void *mem) {
    ...
    if (p->is_mmaped())   /* release mmapped memory. */
    {
...
void *gcmalloc::_int_malloc(mArenaPtr ap, size_t bytes) {
    ...
    */
    ...
    if (nb <= (unsigned long) (DEFAULT_MXFAST)) {
        long int idx = ap->fast_bins.index(nb);
        fb = &((ap->fast_bins)[idx]);
...
//                    alloc_perturb (p, bytes);
            ...
            if (in_smallbin_range(size)) { // 放回small bin, if-else后统一放入
                victim_index = smallbin_idx(size);
                bck = ap->bins.bin_at(victim_index);
                fwd = bck->fd;
            } else {
                victim_index = largebin_idx(size); // 放回largebin
                bck = ap->bins.bin_at(victim_index);
                fwd = bck->fd;
        ...
        */
        ...
        for (;;) {
            ...
            if (bit > map || bit == 0) {
                ...
                bin = ap->bins.bin_at(block << BINMAPSHIFT);
                bit = 1;
            }
...
//                    alloc_perturb(p, bytes);
        ...
        /*
            ...
            (held in ap->topChunk). Note that this is in accord with the best-fit
            search rule.  In effect, ap->topChunk is treated as larger (and thus
            less well fitting) than any other available chunk since it can
            be extended to be as large as necessary (up to system
            limitations).

            We require that ap->topChunk always exists (i.e., has size >=
    ...
    */
    ...
    if ((unsigned long) (size) <= (unsigned long) (DEFAULT_MXFAST)
      ...
      bordering top into fastbins
        */
        && (offset2Chunk(p, size) != ap->topChunk)
#endif
            ) {
        ...
        */
        ...
        /* Lightweight tests: check whether the block is already the
           top block.  */
        if (__builtin_expect(p == ap->topChunk, 0)) {
        ...
        if (__builtin_expect(ap->contiguous()
                             && (char *) nextchunk
                                >= ((char *) ap->topChunk + ap->topChunk->get_size()), 0)) {
        ...
        */
        ...
        if ((unsigned long) (size) >= FASTBIN_CONSOLIDATION_THRESHOLD) {
            ...
            if (ap == &main_arena) { // 主分配区使用sYSTRIm 收缩内存
                if ((unsigned long) (ap->topChunk->get_size()) >=
                    (unsigned long) (gcmp.trim_threshold))
                    trim_to_sys(gcmp.top_pad, ap);
            } else { // 非主分配区始终用heap_trim来收缩内存
                /* Always try heap_trim(), even if the top chunk is not
                   large, because the corresponding heap might go away.  */
                Heap *heap = ap->topChunk->belonged_heap();
...
void gcmalloc::malloc_consolidate(mArenaPtr ap) {
    ...
    */
    ...
    if (DEFAULT_MXFAST != 0) { // 这里是已经被初始化了，如果没有被初始化我们就初始化，然后返回
        ...
        */
        ...
        maxfb = &(ap->fast_bins[NFASTBINS - 1]);
        fb = &(ap->fast_bins[0]);
        do {
            if ((p = *fb) != nullptr) {
                *fb = nullptr;
                ...
                do {
                    ...
                    if (nextchunk != ap->topChunk) { // 下一个chunk 不是top chunk，将合并后的chunk插入到unsorted bin中
                        nextinuse = nextchunk->inuse();
